...
The setup script creates worker/.venv, installs pinned Windows dependencies, prepares hash-verified official checkpoints and fixed640 ONNX artifacts, caches the optional plate artifacts, runs worker tests including actual CUDA parity, and writes a bounded benchmark. It does not train or download datasets.

`ROADLENS_PLATE=auto` enables plate recognition only when both halves are actually prepared, so a worker without them runs exactly the traffic-only pipeline and advertises no plate support. `off` disables it outright — that is the baseline the performance gate in [PLATE_RESULTS.md](PLATE_RESULTS.md) compares against. `on` makes it a hard requirement and refuses to start without it. The plate detector is a locally trained artifact: `training/plates/` builds the dataset, trains it and installs it, and setup will not invent one. Until it exists the worker is traffic-only and reports say "Plate unavailable". Setup caches the OCR weights once; `start.ps1` never downloads anything. Normal start validates the environment/configuration, checks actual CUDA execution, verifies a cached artifact, warms the model and connects to the relay. It does not install packages or export/build engines. Ctrl+C stops the foreground worker; there is no process-name kill script or unrelated-process cleanup.
...
Defaults: ROADLENS_MODEL_MODE=balanced, ROADLENS_GPU_RUNTIME=pytorch_cuda, ROADLENS_GPU_DEVICE=0, ALLOW_WORKER_CPU_FALLBACK=false, ROADLENS_PLATE=auto, ROADLENS_PLATE_OCR=fast-plate-ocr, ROADLENS_PLATE_PREPROCESS=none. Fast/balanced/quality map to tested YOLO26n/s/m640. All numeric inference uses GPU; Python CPU fallback is deliberately unsupported. The UI may use its existing browser WASM detector instead. ONNX CUDA is also tested; TensorRT did not finish the bounded engine build and remains unverified/unselected.
...
| worker.registered `plateProtocol` | Optional `1`, sent only by a relay that understands plate messages. Its absence is how a worker knows to stay silent about its plate pipeline. |
| worker.ready `plate` | Optional. Present only when a plate detector and an OCR engine both loaded **and** the relay advertised `plateProtocol`. `worker.ready` is strict, so announcing it to an older relay would be rejected and drop the worker on every reconnect, costing GPU analysis entirely; negotiating first makes upgrade order harmless. |
| camera.plate | Binary RLP1 magic, four-byte big-endian UTF8 header length, strict v1 header, JPEG bytes. Carries a **vehicle crop taken from the phone's full-resolution frame**, its placement in that frame, a requestId and a trackId. Long edge ≤640, ≥64; target48KiB/hard96KiB; header2048bytes, binary128KiB. |
| plate.result | Same complete request identity plus one single-frame reading: plateText or null, its confidence, detector confidence, the plate box within the crop, and detect/OCR timing. A null reading is a successful observation, not an error. |
| plate.error | Fixed categories only: decode_failed, plate_failed, busy, timeout, unavailable. |
...
Plate work is strictly subordinate and bounded independently at three layers. At most one plate task exists system wide; the worker admits one **only** when no analysis frame is running and none is waiting, so plate work can never queue in front of the next frame. A request arriving at any other moment is refused immediately rather than held. The camera adds its own floors: at most four candidate crops per track, at most eight tracks under analysis, and a 220ms minimum submission interval. The relay times a plate request out at three seconds — and unlike an analysis timeout this does **not** condemn the worker, because taking traffic detection down over an unreadable plate would be the wrong trade; it reports the timeout, disables plate work for that lease, and leaves analysis running.
...
The relay stores bounded memberships, correlation identities, rate limits and transit buffers only. The worker has one transient JPEG/tensor/result, no media/report archive. A plate request is the same: the crop, the reading and the request identity exist for one call and are then gone, so the worker holds no plate text at all and multi-frame agreement is the camera's job. Worker logs contain categorical status and model/runtime diagnostics, never codes, capabilities, images, report facts or plate text. The only deliberate persistent outputs are model/engine caches, explicit benchmark/test artifacts and user report downloads. Reports/evidence remain bounded camera/viewer RAM.
