...
| worker.ready `plate` | Optional. Present only when a plate detector and an OCR engine both loaded, carrying detector id/hash, OCR engine name and input size. Omitted entirely otherwise, so a worker without plate support sends exactly what it always sent. |
